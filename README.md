# Deep Research Agent（课程学习研究助手）

> LangGraph 编排 + smolagents 内核的「规划-执行-反思」三节点 Agent。
> **业务场景**：面向高校计算机专业学生的课程学习研究助手（教育垂直场景）——
> 输入一个课程问题，自动规划 → 多轮搜索（含本地课程知识库）→ 反思 → 输出带来源引用的结构化研究报告。
> 详细场景定位（用户需求/差异化/效果评估）见 `docs/BUSINESS_SCENARIO.md`。
...
> 说明：重跑后 14/16 条关键词全命中；id=3（兴趣）/id=9（健康）各 0.5（未全命中），均跑满 5 步 + 护栏强制收尾，属 LLM 输出随机性的单次样本波动（temperature=0.3，非早停漏词）。**严谨表述**：94% 为单次运行结果，命中率随模型输出有 ±0.1~0.2 的波动，多次重跑取平均会更稳（评测成本约 6 元/轮，迭代中已多轮重跑验证方向稳定）；指标卡建议写「命中率 94%（单次），波动主要来自 LLM 随机性」。**循环轮数 3.1→3.5 是 iteration 修复的预期效果**（修复前 replan 重置计数导致轮数被低估）；耗时下降 31% 主要来自 Bing 响应波动。评测集已扩展至 17 条（含 #17 注入安全用例，待重跑出数据）。
...
> 面向"可部署、可维护、可演进"的工程化设计，而非一次性脚本。对应代码：`deep_research/` 包 + `tests/`（91 个用例，全部 mock，秒级跑完，不依赖真实 API/网络）。
